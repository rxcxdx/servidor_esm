...
export async function findRelatorio(gteISOString, lteISOString, username) {
  const filtro = {
    dt: {
      $gte: dayjs(gteISOString).startOf('day').toDate(),
      $lte: dayjs(lteISOString).endOf('day').toDate()
    }
  }
  if (check.nonEmptyString(username)) {
    logger.debug('filtrei username')
...
export async function apagar(v) {
